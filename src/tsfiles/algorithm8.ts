...
export function algorithm8(width:number, height:number){
  const circleNum = Math.random()*50;
  var returnData: drawCircles[] = []
  for(let i = circleNum; i > 0; i--){
      const newData = new drawCircles(Math.random()*width, Math.random()*height, Math.random()*90 + 30, Math.random()*45 + 245, Math.random()*80 + 20, Math.random()*10 + 90, Math.random()*20)
      const count = 1000
